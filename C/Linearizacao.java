...
public class Linearizacao {
  ...
  static int altura = 916, largura = 1182, cel_side = 50, x, y, dilatacao;
  ...
  private static LinkedList<Pos> lineariza (LinkedList<Pos> path) {
    Pos p, q, old_q = null;
    LinkedList<Pos> linearPath = null;
    ...
    while (!path.isEmpty()) {
      ...
      while (!intercepta) {
        ...
        intercepta = false;
        Line pq = new Line ((p.x()+0.5f)*cel_side, (p.y()+0.5f)*cel_side, (q.x()+0.5f)*cel_side, (q.y()+0.5f)*cel_side);
        for (Line l : lines) {
          if (pq.intersectsAt(l) != null) {
            intercepta = true;
          }
        }
      }
