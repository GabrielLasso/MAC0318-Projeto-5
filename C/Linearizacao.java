...
// Igual ao Thickening, só que lineariza o caminho depois

public class Linearizacao {
  static boolean[][] mapa;
  ...
  private static int altura = 916, largura = 1182, cel_side = 50, dilatacao = 56;
  ...
  private static LinkedList<Pos> lineariza (LinkedList<Pos> path) {
    ...
    while (!path.isEmpty()) {
      ...
      while (!intercepta) {
        ...
        intercepta = false;
        Line pq = new Line ((p.x()+0.5)*cel_side, (p.y()+0.5)*cel_side, (q.x()+0.5)*cel_side, (q.y()+0.5)*cel_side);
        for (Line l : lines) {
          if (pq.intersects(l)) {
            intercepta = true;
          }
  ...
  public static void main (String[] args) {
    ...
    master.connect();
    LinkedList<Pos> path, linearPath;
    Pos start, goal;
    ...
    linearPath = lineariza (path);
    while (!linearPath.isEmpty()) {
      Pos pos = linearPath.removeFirst();
